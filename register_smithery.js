...
// 엔드포인트 업데이트 - 루트 경로 사용
smitheryJson.transport.jsonrpc.endpoint = "";
...
try {
  ...
  console.log(`API 엔드포인트 테스트 명령: ${curlCmd}`);
  const curlOutput = execSync(curlCmd, { encoding: 'utf8' });
  console.log('API 응답:', curlOutput);
} catch (error) {
...
try {
  ...
  const listOutput = execSync('npx @smithery/cli list servers --client cursor', { encoding: 'utf8' });
